...
im = imageio.imread('mpmsim/pneunetsmall.png')[:,:,0]
...
occ = np.array(occ)
mat = np.array(mat)
n_particles = len(occ[:]) * quality**2
n_grid = im.shape[0] * quality # ONLY USES THE X DIMENSION OF THE IMAGE
##
...
## Apply the coordinates and materials
x.from_numpy(occ)
...
initialize()
gui = ti.GUI("Taichi MLS-MPM-99ai", res=im.shape[1], background_color=0x112F41)
